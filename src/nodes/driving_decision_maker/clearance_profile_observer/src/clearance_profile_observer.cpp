...
//            //  Subscriptions for signal detection
//         signal_detected_subscription_ = this->create_subscription<std_msgs::msg::Bool>(
//             "signal_detected", 10,
//             std::bind(&ClearanceObserver::signalDetectedCallback, this, std::placeholders::_1));
...
//         // Publisher for recolored clearance tubes
//         recolored_clearance_tube_publisher_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(
//             "recolored_clearance_tubes", 10);  
...
//             visualization_msgs::msg::Marker transformed_marker;
//             if (transformBoundingBoxToMapFrame(marker, transformed_marker)) {
//                 bool is_in_path = checkCollision(transformed_marker, intersection_point);
...
//                         }
//                     }
//                      recolorTubeFromIntersection(intersection_point);
//                 } else {
//                     // Color in grey if not inside the path
//                     transformed_marker.color.r = 0.5;
//                     transformed_marker.color.g = 0.5;
...
//     void recolorTubeFromIntersection(const geometry_msgs::msg::Point& intersection_point) {
//     for (auto& tube_marker : clearance_profile_tube_.markers) {
//         for (size_t i = 0; i < tube_marker.points.size(); i += 2) {
//             const auto& p1 = tube_marker.points[i];
//             const auto& p2 = tube_marker.points[i + 1];

//             // Check if the tube section contains the intersection point
//             if (pointLineDistance(intersection_point, p1, p2) < tube_marker.scale.x) {
//                 // Change the color of the tube from this point onward
//                 tube_marker.color.r = 1.0;  // Red for example
//                 tube_marker.color.g = 0.0;
//                 tube_marker.color.b = 0.0;
//                 tube_marker.color.a = 0.5;  // 50% visibility

//                 RCLCPP_INFO(this->get_logger(), "Recoloring tube section from intersection point.");
//             }
//         }
//     }
//     // Publish the updated tube markers
//     recolored_clearance_tube_publisher_->publish(clearance_profile_tube_);
// }
...
//     bool checkCollision(const visualization_msgs::msg::Marker& bounding_box, geometry_msgs::msg::Point& intersection_point) {
//         // Convert Pose to TransformStamped
//         geometry_msgs::msg::TransformStamped transform_stamped;
//         transform_stamped.header = bounding_box.header;
//         transform_stamped.transform.translation.x = bounding_box.pose.position.x;
...
//         // Check each transformed corner against the clearance profile tube
//         for (const auto& corner : corners) {
//             if (isPointInsideTube(corner, clearance_profile_tube_, intersection_point)) {
//                 return true;  // Collision detected
//             }
//         }
...
//     bool isPointInsideTube(const geometry_msgs::msg::Point& point, const visualization_msgs::msg::MarkerArray& tube, geometry_msgs::msg::Point& intersection_point) {
//         for (const auto& tube_marker : tube.markers) {
//             for (size_t i = 0; i < tube_marker.points.size(); i += 2) {
//                 const auto& p1 = tube_marker.points[i];
//                 const auto& p2 = tube_marker.points[i + 1];
...
//                 double distance = pointLineDistance(point, p1, p2);
//                 if (distance < tube_marker.scale.x) {  // tube_marker.scale.x is used as the radius here
//                 intersection_point = point;  // Store intersection point
//                     return true;
//                 }
//             }
//         }
...
//     rclcpp::Publisher<observer_msgs::msg::ObserverInfo>::SharedPtr observer_info_publisher_;  
//     rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr filtered_bounding_boxes_publisher_;  
//     rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr recolored_clearance_tube_publisher_;
...
//      void velocityCallback(const geometry_msgs::msg::Twist::SharedPtr msg) {
//         current_velocity_ = msg->linear.x;  // Assuming forward velocity is in the x direction
//     }
...
//         // Recolor the tube from the smoothed distance onward
//         recolorTubeFromSmoothedDistance(smoothed_distance);
//     }
...
//     void recolorTubeFromSmoothedDistance(double smoothed_distance) {
//     double accumulated_distance = 0.0;  // To track the distance traveled along the tube
//     bool recoloring_started = false;    // To check if recoloring has started

//     // Loop through each tube marker
//     for (auto& tube_marker : clearance_profile_tube_.markers) {
//         // Iterate through points in pairs (each tube segment is represented by two points)
//         for (size_t i = 0; i < tube_marker.points.size(); i += 2) {
//             const auto& p1 = tube_marker.points[i];
//             const auto& p2 = tube_marker.points[i + 1];

//             // Calculate the length of the current tube segment
//             double segment_length = pointLineDistance(p1, p1, p2);  // Length of the segment

//             // Check if the current segment crosses the smoothed distance
//             if (accumulated_distance + segment_length >= smoothed_distance) {
//                 // Calculate how much remaining distance before we hit the smoothed_distance
//                 double remaining_distance = smoothed_distance - accumulated_distance;

//                 // Get the point where recoloring should start
//                 geometry_msgs::msg::Point recolor_start_point = getPointAlongLine(p1, p2, remaining_distance);

//                 // Start recoloring the tube starting from this point
//                 recolorTubeSegmentFrom(recolor_start_point, tube_marker, i);

//                 RCLCPP_INFO(this->get_logger(), "Recoloring tubes from distance: %f", smoothed_distance);
//                 recoloring_started = true;
//                 break;  // Exit the loop once recoloring starts
//             }
...
//         if (recoloring_started) {
//             break;  // Exit if recoloring has started
//         }
//     }

//     // If recoloring has started, publish the modified tube markers
//     if (recoloring_started) {
//         RCLCPP_INFO(this->get_logger(), "Publishing recolored tubes.");
//         recolored_clearance_tube_publisher_->publish(clearance_profile_tube_);
//     } else {
//         RCLCPP_WARN(this->get_logger(), "Recoloring did not start as expected.");
//     }
// }



//     void recolorTubeSegmentFrom(const geometry_msgs::msg::Point& start_point, visualization_msgs::msg::Marker& tube_marker, size_t start_index) {
//     bool recolor_started = false;

//     for (size_t i = start_index; i < tube_marker.points.size(); i += 2) {
//         const auto& p1 = tube_marker.points[i];
//         const auto& p2 = tube_marker.points[i + 1];

//         // If recoloring hasn't started yet, check if the start point falls within this segment
//         if (!recolor_started) {
//             if (pointLineDistance(start_point, p1, p2) < tube_marker.scale.x) {
//                 recolor_started = true;  // Start recoloring from here
//             }
//         }

//         // Once recoloring has started, set the tube segment color to red
//         if (recolor_started) {
//             tube_marker.color.r = 1.0;  // Red color starting at the object
//             tube_marker.color.g = 0.0;
//             tube_marker.color.b = 0.0;
//             tube_marker.color.a = 0.5;  // Example: 50% visibility
//         } else {
//             // Keep the original color before recoloring starts (green as an example)
//             tube_marker.color.r = 0.0;
//             tube_marker.color.g = 1.0;
//             tube_marker.color.b = 0.0;
//             tube_marker.color.a = 1.0;  // Full visibility for the original color
//         }
//     }
// }


//     geometry_msgs::msg::Point getPointAlongLine(const geometry_msgs::msg::Point& p1, const geometry_msgs::msg::Point& p2, double distance) {
//     geometry_msgs::msg::Point point;
//     double segment_length = pointLineDistance(p1, p1, p2);

//     double t = distance / segment_length;  // Ratio of the distance along the segment
