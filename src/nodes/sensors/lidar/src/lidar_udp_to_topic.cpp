...
private:
    ...
    uint16_t previous_frame_id_groundTruth_ = 0;        // ID of receiving upd pointcloud
    ...
    bool convertToPointCloud(const char *data, size_t size)
    {
        ...
        else if (tokens[0] == "2")
        {
            ...
            // process payload data
            uint16_t frame_id_groundTruth = std::stoi(tokens[1]);
            // Check if ID has changed
            if (frame_id_groundTruth != previous_frame_id_groundTruth_)
            {
            ...
            for (size_t j = 4; j < tokens.size(); ++j)
            {
                ...
                while (std::getline(object_iss, object_token, ' '))
                { // Assuming space is the secondary delimiter
                ...
                groundtruth_msg.id = frame_id_groundTruth;
                groundtruth_msg.centerx = std::stod(object_tokens[1]) / 100;
                groundtruth_msg.centery = std::stod(object_tokens[0]) / 100; // 180° y mirroring
            ...
            previous_frame_id_groundTruth_ = frame_id_groundTruth;
        }
        else
        {
    ...
    void publishPointcloud(const std::vector<double> &pointcloud_data)
    {
        ...
        ;                                                                           // Number of points, each consits of 3 coordinates
    ...
    void publishBoundingBoxes(const std::vector<lidar_msgs::msg::GroundTruth> &buffer)
    {
        ...
        delete_marker.header.frame_id = "lidar_frame";
        ...
        {
            const auto &groundTruth = buffer[i];
 
            visualization_msgs::msg::Marker boundingbox;
            boundingbox.header.stamp = this->now();
            boundingbox.header.frame_id = "lidar_frame";
            boundingbox.ns = "boxes";
            boundingbox.id = groundTruth.id * 100 + i;
            ...
            tf2::Quaternion quat;
            quat.setRPY(0, 0, groundTruth.yaw * 3.14 / 180.);
            ...
            {
                boundingbox.color.a = 0.0; // Transparency
                boundingbox.color.r = 0.0;
                boundingbox.color.g = 0.0;
                boundingbox.color.b = 0.0;
            }
...
//OLD WORKING VVERSION without bounding box buffering and marker delete #include "rclcpp/rclcpp.hpp"
// #include "sensor_msgs/msg/point_cloud2.hpp"
// #include <netinet/in.h>
// #include <sys/socket.h>
// #include <unistd.h>
